...
class Block:
    ...
    def smash(self) -> bool:
        ...
        """
        # if len(self.children) == 4 or self.level >= self.max_depth:
        if not self.smashable():
            return False
        else:
            self.colour = None
            c1 = COLOUR_LIST[random.randint(0, len(COLOUR_LIST) - 1)]
            c2 = COLOUR_LIST[random.randint(0, len(COLOUR_LIST) - 1)]
            c3 = COLOUR_LIST[random.randint(0, len(COLOUR_LIST) - 1)]
            c4 = COLOUR_LIST[random.randint(0, len(COLOUR_LIST) - 1)]
            self.children = [Block(self._children_positions()[0],
                                   self._child_size(), c1, self.level+1,
                                   self.max_depth),
                             Block(self._children_positions()[1],
                                   self._child_size(), c2, self.level+1,
                                   self.max_depth),
                             Block(self._children_positions()[2],
                                   self._child_size(), c3, self.level+1,
                                   self.max_depth),
                             Block(self._children_positions()[3],
                                   self._child_size(), c4, self.level+1,
                                   self.max_depth)]
            for child in self.children:
                r = random.random()
                if r < math.exp(-0.25 * self.level):
                    child.smash()
            return True
    ...
    def swap(self, direction: int) -> bool:
        ...
        if len(self.children) == 0:
            ...
            if direction == 1:
                c0._update_children_positions(cp[3])
                c1._update_children_positions(cp[2])
                c2._update_children_positions(cp[1])
                c3._update_children_positions(cp[0])
                self.children = [c3, c2, c1, c0]
            else:
                c0._update_children_positions(cp[1])
                c1._update_children_positions(cp[0])
                c2._update_children_positions(cp[3])
                c3._update_children_positions(cp[2])
                self.children = [c1, c0, c3, c2]
            return True
    ...
    def combine(self) -> bool:
        ...
        if len(c_oc) == 4:
            ...
            self.colour = self.children[0].colour
            self.children = []
            return True
    ...
    def create_copy(self) -> Block:
        ...
        """
        c_block = Block(self.position, self.size, self.colour,
                        self.level, self.max_depth)
        if len(self.children) == 0:
            c_block.children = []
        else:
            c_block.children.append(self.children[0].create_copy())
            c_block.children.append(self.children[1].create_copy())
            c_block.children.append(self.children[2].create_copy())
            c_block.children.append(self.children[3].create_copy())
        return c_block
...
if __name__ == '__main__':
    ...
    print(b1)
    print(id(b1))
    print("=== bc before smash ===")
    print(bc)
    print(id(bc))
    b1.smash()
    bc = b1.create_copy()
    print("=== tiny board smashed ===")
    print(b1)
    print(id(b1))
    print(id(b1.children[2]))
    print("=== bc after smash ===")
    print(bc)
    print(id(bc))
    print(id(bc.children[2]))
